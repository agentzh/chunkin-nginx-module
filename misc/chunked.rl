...
int main (int argc, char **argv) {
    ...
    size_t bytes_read;
    eof = NULL;
    size_t data_bytes_read, chunk_size;
    ...
    %%{
        ...
        action bad_chunk_data {
            fprintf(stderr, "bad chunk data (bytes already read %d, bytes expected: %d): offset %d.\n", data_bytes_read, chunk_size, p - buf);
        }
        ...
        action verify_data {
            if (data_bytes_read != chunk_size) {
                fprintf(stderr, "ERROR: chunk size not meet: %d != %d\n", data_bytes_read, chunk_size);
                fbreak;
            }
        }
        ...
        chunk_data_octet = any ${fprintf(stderr, "data bytes read: %d (char: %c)\n", data_bytes_read, *p);} when test_len $err(bad_chunk_data) $read_data;
    ...
    while ((bytes_read =
            fread(buf + len, sizeof(char), BUFSIZE, stdin)) > 0) {
        len += bytes_read;
    ...
    printf(">= first_final %d, execute = %i, p moved %d, p remaining: %d\n", cs >= foo_first_final, res, p - buf, pe - p);
